...
for i in range(ncells):
    ...
    gdpnetper[i] = (
        float(line[-1]) * 1e-3
    )  # NB!! To be consistent with number of decimals Henri use
...
def get_chit():
    ...
    chi = (1 + np.exp(np.log(0.01 / 0.99) * (years - n_05) / (n_01 - n_05))) ** (-1)
    chit = chi / chi[0]
    chit[110:] = 0
...
def get_price():
    ...
    cumulative_emissions = np.loadtxt(
        file_path + "NorESM2_HIST_SSP370_cumulative_emissions_global_temperature.txt",
        usecols=1,
        comments="#",
    )
...
def get_initial_ai():
    ...
    ai = (
        b * (1 - theta) * (capitali ** (alpha * theta)) * (xi ** (-theta)) / price
    ) ** (1 / (theta * (alpha - 1)))
...
def get_pi_temperature():
    pi_temperature = np.loadtxt(
        file_path + "NorESM2_picontrol_regional_temperatures.txt",
        usecols=3,
        comments="#",
    )
...
def get_year_current(case_name):
    ...
    file = (
        "/cluster/work/users/jennybj/noresm/" + case_name + "/run/rpointer.atm"
    )  # CHANGE path
...
def damage_function(temp, temp_opt, kappa_plus, kappa_minus):
    ...
    for i in range(N):
        if temp[i] > temp_opt:
            productivity[i] = (
                (1 - lower_bound) * np.exp(-kappa_plus * (temp[i] - temp_opt) ** 2)
                + lower_bound
            ) ** (1 / (1 - alpha))

        elif temp[i] <= temp_opt:
            productivity[i] = (
                (1 - lower_bound) * np.exp(-kappa_minus * (temp[i] - temp_opt) ** 2)
                + lower_bound
            ) ** (1 / (1 - alpha))
...
def regrid_from_noresm_to_diam(noresm_variable):
    ...
    interp_lat = np.arange(-90.0, 90.0, 1)
...
def from_diam_order_to_grid(in_variable):
    ...
    out_latitudes = np.arange(-90.0, 90.0, 1)
    out_longitudes = np.arange(-180.0, 180.0, 1)
...
def get_noresm_regional_temperatures(year_current, case_name):
    ...
    global_temp = np.average(
        np.average(regtemp, axis=1),
        axis=0,
    ...
    print("NorESM2 temperatures read in: ", datetime.timestamp(datetime.now()) - ts)
...
def calculate_annual_mean(variable):
    ...
    for iyear in range(nyears):
        out[iyear] = np.average(
            variable[iyear * 12 : (iyear + 1) * 12], axis=0, weights=month_lengths
        )
...
def write_to_txt_file_gridded(year_current, file_name, variable):
    ...
    save_kwargs = dict(fmt="%16.8f", delimiter="", newline="", footer="\n", comments="")
...
def make_emissions_file(case_name, emissions_file):
    ...
    # File names:
    outfile = (
        "/cluster/home/jennybj/input_emissions_" + case_name + ".nc"
    )  # CHANGE path to that specified by "co2flux_fuel_file" in "user_nl_cam"
    ...
    for i in range(gridded_co2.shape[0]):
        smoothing = gaussian_filter(
            interp_co2[i, :, :], sigma=1
        )  # to improve interpolation

        f = RegularGridInterpolator(
            (interp_lat, interp_lon), smoothing, bounds_error=False, fill_value=None
        )
        interpolated = f((X, Y))
    ...
    print("Created emissions file in: ", datetime.timestamp(datetime.now()) - ts)
