...
ssp_co2_files = [
    path
    + "emissions-cmip6_CO2_anthro_surface_ScenarioMIP_IAMC-IMAGE-ssp126_201401-210112_fv_1.9x2.5_c20221115.nc",
    path
    + "emissions-cmip6_CO2_anthro_surface_ScenarioMIP_IAMC-MESSAGE-GLOBIOM-ssp245_201401-210112_fv_1.9x2.5_c20230419.nc",
    path
    + "emissions-cmip6_CO2_anthro_surface_ScenarioMIP_IAMC-AIM-ssp370_201401-210112_fv_1.9x2.5_c20190207.nc",
    path
...
in_co2_hist = ncfile.variables["CO2_flux"][
    100 * 12 : -1 * 12
]  # last year repeated (not include 2015)
...
for file in ssp_co2_files:
    ncfile = Dataset(file)
    in_co2_ssps.append(ncfile.variables["CO2_flux"][12:-12])  # use 2014 from historical
    ncfile.close()
...
for i, run in enumerate(runs):
    ...
    if i == 0:
        cumulative_co2_noresm = np.concatenate((np.array([0]), np.cumsum(co2_noresm)))
    else:
        cumulative_co2_noresm = np.cumsum(co2_noresm) + cumulative_co2_noresm_hist[-1]
...
for name in names[1:]:
    ...
    exec(
        "cumulative_co2_noresm_"
        + name
        + "= np.concatenate((cumulative_co2_noresm_hist, cumulative_co2_noresm_"
        + name
        + "))"
    )
