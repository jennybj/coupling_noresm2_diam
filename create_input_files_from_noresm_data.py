...
pi_temp = np.average(
    np.average(in_temperature_pi, axis=1),
    axis=0,
...
for i in range(1, len(in_temperature)):
    ...
    temperatures.append(
        sort_in_diam_order_3D(diam_latitudes, diam_longitudes, temp)
    )  # T it
...
file = open("NorESM2_HIST_SSP370_cumulative_emissions_global_temperature_v3.txt", "w")
...
for icell in range(ncells):
    y = np.concatenate(
        (
            temperatures[1][icell, :] - temperature_pi[icell],
            temperatures[2][icell, :] - temperature_pi[icell],
            temperatures[0][icell, :] - temperature_pi[icell],
        )
    )
...
for icell in range(ncells):
    ...
    expected_temperature_e1[icell, :] = (
        ...
        + coeffs[icell, 1] * cumulative_co2_e1[:-1] ** 2
    )
    expected_temperature_e2[icell, :] = (
        coeffs[icell, 0] * cumulative_co2_e2[:-1]
        + coeffs[icell, 1] * cumulative_co2_e2[:-1] ** 2
    )
...
for icell in range(ncells):
    zi_all[icell, :] = (
        temperatures[0][icell, :]
        - temperature_pi[icell]
        - expected_temperature_all[icell, :]
    )
    zi_e1[icell, :] = (
        temperatures[1][icell, :]
        - temperature_pi[icell]
        - expected_temperature_e1[icell, :]
    )
    zi_e2[icell, :] = (
        temperatures[2][icell, :]
        - temperature_pi[icell]
        - expected_temperature_e2[icell, :]
    )
