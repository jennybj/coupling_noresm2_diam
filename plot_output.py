...
for iyear in range(nyears):
    ...
    sum_gdp_detrended[iyear] = sum_gdp[iyear] / (1 + ga) ** iyear
    fp_gdp_detrended[iyear, :] = fp_gdp[iyear, :] / (1 + ga) ** iyear
    fp_sum_gdp_detrended[iyear] = fp_sum_gdp[iyear] / (1 + ga) ** iyear

print("Percentage change in GDP (detrended):")
print(
    100 * (sum_gdp_detrended - sum_gdp_detrended[0]) / sum_gdp_detrended[0],
    np.mean(
        100 * (sum_gdp_detrended[-10:] - sum_gdp_detrended[0]) / sum_gdp_detrended[0]
    ),
)
print(
    100 * (fp_sum_gdp_detrended - fp_sum_gdp_detrended[0]) / fp_sum_gdp_detrended[0],
    np.mean(
        100
        * (fp_sum_gdp_detrended[-10:] - fp_sum_gdp_detrended[0])
...
pop_temp = np.average(temperature - pi_temperatures, axis=1, weights=population)
expected_pop_temp = np.average(
    expected_temperature - pi_temperatures, axis=1, weights=population
)
...
for c, country in enumerate(chosen_countries):
    ...
    pi_temp_countries[c] = calculate_regional_mean(
        pi_temperatures[:], indices, weights=pops
    )

    for iyear in range(nyears):
        temp_countries[iyear, c] = calculate_regional_mean(
            temperature[iyear, :], indices, weights=pops
        )
...
for iyear in range(nyears):  # nyears):
    ...
    pscat3 = ax4[0].scatter(
        np.average(expected_temperature[iyear, :], weights=population)
        - np.average(pi_temperatures, weights=population),
        100
        * (fp_sum_gdp_detrended[iyear] - fp_sum_gdp_detrended[0])
        / fp_sum_gdp_detrended[0],
        c="black",
        s=50,
        alpha=0.8,
    ...
    pscat4 = ax4[1].scatter(
        ...
        - np.average(pi_temperatures, weights=population),
        100 * (sum_gdp_detrended[iyear] - sum_gdp_detrended[0]) / sum_gdp_detrended[0],
        c="black",
        s=50,
        alpha=0.8,
    )
    ...
    fig4.savefig("figures/countries_year_{:d}.png".format(1990 + iyear))
...
for idec in range(1, ndecades1):
    ...
    expected_dtemp_countries = (
        np.average(expected_temp_countries[10 * idec : 10 * (idec + 1), :], axis=0)
        - expected_temp_start
    )
    dtemp_countries = (
        np.average(temp_countries[10 * idec : 10 * (idec + 1), :], axis=0) - temp_start
    )

    print(
        np.max(expected_dtemp_countries),
    ...
    dgdp_countries = (
        100
        * (np.average(gdp_country[10 * idec : 10 * (idec + 1), :], axis=0) - gdp_start)
        / gdp_start
    )
    ...
    for c, country in enumerate(all_countries):
        if country in text_countires:
            # Add country names:
            ax5.text(
                expected_dtemp_countries[c],
                expected_dgdp_countries[c],
                country,
                fontsize=5,
            )
            ax6.text(dtemp_countries[c], dgdp_countries[c], country, fontsize=5)
            """
            # Add circle around chosen courtries:
    ...
    ax5.scatter(
        np.average(
            np.average(
                expected_temperature[10 * idec : 10 * (idec + 1), :],
                weights=population,
                axis=1,
            )
        )
        - np.average(
            np.average(expected_temperature[:10, :], weights=population, axis=1)
        ),
        100
        * (
            np.average(fp_sum_gdp_detrended[10 * idec : 10 * (idec + 1)])
    ...
    ax6.scatter(
        np.average(
            np.average(
                temperature[10 * idec : 10 * (idec + 1), :], weights=population, axis=1
            )
        )
        - np.average(np.average(temperature[:10, :], weights=population, axis=1)),
        100
        * (
    ...
    fig5.savefig(
        "figures/country_gdp_change_expectation_same_axes_{:d}_{:d}.pdf".format(
            1990 + idec * 10, 1999 + idec * 10
        )
    )
    fig5.savefig(
        "figures/country_gdp_change_expectation_same_axes_{:d}_{:d}.png".format(
            1990 + idec * 10, 1999 + idec * 10
        )
    )
    fig6.savefig(
        "figures/country_gdp_change_damage_noresm2-diam_same_axes_{:d}_{:d}.pdf".format(
            1990 + idec * 10, 1999 + idec * 10
        )
    )
    fig6.savefig(
        "figures/country_gdp_change_damage_noresm2-diam_same_axes_{:d}_{:d}.png".format(
            1990 + idec * 10, 1999 + idec * 10
        )
    )
...
ax8[0].plot(
    years[:-1],
    np.percentile(fp_gdp_detrended / population, 75, axis=1)
    / np.percentile(fp_gdp_detrended / population, 25, axis=1),
    label="DIAM fixed point",
)
ax8[0].plot(
    years[:-1],
    np.percentile(gdp_detrended / population, 75, axis=1)
    / np.percentile(gdp_detrended / population, 25, axis=1),
    label="NorESM2-DIAM",
)
...
ax8[1].plot(
    years[:-1],
    np.percentile(fp_gdp_detrended / population, 90, axis=1)
    / np.percentile(fp_gdp_detrended / population, 10, axis=1),
    label="DIAM fixed point",
)
ax8[1].plot(
    years[:-1],
    np.percentile(gdp_detrended / population, 90, axis=1)
    / np.percentile(gdp_detrended / population, 10, axis=1),
    label="NorESM2-DIAM",
)
...
ax8.plot(
    years[:-1],
    np.std(fp_gdp_detrended / population, axis=1),
    label="DIAM expectation",
)
ax8.plot(years[:-1], np.std(gdp_detrended / population, axis=1), label="NorESM2-DIAM")
